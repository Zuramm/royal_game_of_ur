...
@export var width: float = 1:
	set(value):
		width = maxf(thickness * 2.0, value)
		if Engine.is_editor_hint():
			update_mesh()
@export var height: float = 1:
	set(value):
		height = maxf(thickness * 2.0, value)
		if Engine.is_editor_hint():
			update_mesh()
@export var corner_radius: float = .2:
	set(value):
		corner_radius = clampf(value, thickness, minf(width, height) / 2.0)
		if Engine.is_editor_hint():
			update_mesh()
...
func update_mesh() -> void:
	...
	for r in range(3, -1, -1):
		...
		for i in range(0, 9):
			...
			immediate_mesh.surface_add_vertex((corner - corner_radius_3) * m + v * corner_radius)
			immediate_mesh.surface_add_vertex((corner - corner_radius_3) * m + v * (corner_radius - thickness))
	
	immediate_mesh.surface_add_vertex(Vector3(-width / 2.0 + corner_radius, height / 2.0, 0.0))
	immediate_mesh.surface_add_vertex(Vector3(-width / 2.0 + corner_radius, height / 2.0 - thickness, 0.0))
	...
	for i in range(0, ceilf((width + height - thickness_diag * 2.0) / (thickness_diag * 2.0))):
		...
		immediate_mesh.surface_add_vertex(corner_fill + clamp_diag(p0))
		immediate_mesh.surface_add_vertex(corner_fill + clamp_diag(p1))
		immediate_mesh.surface_add_vertex(corner_fill + clamp_diag(p2))
		immediate_mesh.surface_add_vertex(corner_fill + clamp_diag(p3))
